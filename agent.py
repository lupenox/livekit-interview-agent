...
def _log_required_configuration() -> None:
    ...
    if missing:
        ...
        logger.info(
            "Required LiveKit, Deepgram, ElevenLabs, and Groq keys are configured"
        )
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    text, Groq produces the interviewer response, and ElevenLabs publishes the
    ...
    session = AgentSession(
        ...
        # LLM: transcribed text + current stage instructions -> interviewer text.
        # Groq exposes an OpenAI-compatible API, so LiveKit's OpenAI plugin can
        # point at Groq's base URL while keeping the rest of the agent unchanged.
        llm=openai.LLM(
            model=GROQ_MODEL,
    ...
    try:
        ...
        logger.info("Voice session started with Deepgram STT → Groq → ElevenLabs TTS")
    except Exception:
